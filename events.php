...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'book_event'){
  ...
  if(empty($name_err) && empty($email_err) && empty($contact_err) && empty($capacity_err)) {
    $sql = "INSERT INTO user_booking (name, email, contact, event_id, capacity, type) VALUES (?, ?, ?, ?, ?, ?)";

    if($stmt = $mysqli->prepare($sql)){
        $stmt->bind_param("ssssss", $param_name, $param_email, $param_contact, $param_event_id, $param_capacity, $param_type);

        // parameters
        $param_name = $name;
        $param_email = $email;
        $param_contact = $contact;
        $param_event_id = $_POST['form_id'];
                ...
                <div class="col-md-4 mb-3" style="margin-right: 16px;">
                  <div class="card" style="border-radius: 16px">
                    ...
                    <div class="card-body">
                      ...
                      <div>
                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#reserve_<?php echo str_replace(' ', '-', strtolower($row['name'])) ?>">Reserve</button>
                      </div>
                    </div>
                  </div>
                </div>
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form action="<?php echo htmlspecialchars("/"); ?>" method="post" enctype="multipart/form-data">
                                    ...
                                    <input name="action" value="book_event" class="hidden">
